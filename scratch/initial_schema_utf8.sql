...
-- ============================================================
-- entity_media_type  ΓÇö REMOVED: entity_media now uses explicit FKs (Change 1)
-- proposal_source_type ΓÇö REMOVED: proposals now uses explicit FKs (Change 1)
...
CREATE TABLE posts (
  ...
  -- CHANGE 5: tags TEXT[] column REMOVED ΓÇö replaced by post_tags junction table
...
-- ============================================================
-- TABLE: products
-- CHANGE 3: publish BOOLEAN ΓåÆ status product_status ENUM
-- CHANGE 5: tags TEXT[] REMOVED ΓåÆ product_tags junction table
-- ============================================================
CREATE TABLE products (
  ...
  description   TEXT,
  category_id   UUID            REFERENCES categories(id) ON DELETE SET NULL,
  -- CHANGE 5: tags TEXT[] column REMOVED ΓÇö replaced by product_tags junction table
...
COMMENT ON TABLE products IS
  'Portfolio product entries.
   ...
   status       = product_status ENUM (draft, published, archived) ΓÇö replaces publish BOOLEAN.
   Tags stored in product_tags junction table (Change 5).
   Media gallery stored in entity_media (using explicit product_id FK ΓÇö Change 1).
...
-- ============================================================
-- TABLE: projects
-- CHANGE 3: publish BOOLEAN ΓåÆ status project_status ENUM
-- CHANGE 5: tags TEXT[] REMOVED ΓåÆ project_tags junction table
-- ============================================================
CREATE TABLE projects (
  ...
  description   TEXT,
  category_id   UUID            REFERENCES categories(id) ON DELETE SET NULL,
  -- CHANGE 5: tags TEXT[] column REMOVED ΓÇö replaced by project_tags junction table
  featured      BOOLEAN         NOT NULL DEFAULT FALSE,
  -- CHANGE 3: replaced "publish BOOLEAN" with status ENUM
...
COMMENT ON TABLE projects IS
  'Portfolio project entries.
   status       = project_status ENUM (ongoing, completed, archived) ΓÇö replaces publish BOOLEAN.
   Tags stored in project_tags junction table (Change 5).
   Media gallery in entity_media (using explicit project_id FK ΓÇö Change 1).
...
-- ============================================================
-- TABLE: entity_media
-- CHANGE 1: Polymorphic ΓåÆ Exclusive Arcs with explicit FKs
-- ============================================================
...
COMMENT ON TABLE entity_media IS
  'Unified ordered media gallery for products and projects.
   ...
   Native ON DELETE CASCADE handles cleanup ΓÇö no triggers needed.
...
COMMENT ON TABLE product_modules IS
  'Each product can have a variable number of modules.
   description is unrestricted TEXT ΓÇö rich formatting lives at the app layer.
...
COMMENT ON TABLE position_items IS
  'Merged replacement for position_responsibilities, position_requirements, and position_benefits.
   ...
   display_order is unique per (position, kind) ΓÇö deferrable to allow bulk reorder in one txn.';

CREATE INDEX idx_position_items_position ON position_items (position_id, kind, display_order);

-- ============================================================
-- TABLE: team_members
-- CHANGE 6: role TEXT ΓåÆ position_id FK to positions table
-- ============================================================
...
COMMENT ON TABLE admin_users IS
  'Application-level admin profile extending auth.users.
   role = superadmin ΓåÆ can edit all sections, create/update/delete other admins, and assign permissions.
   role = editor / viewer ΓåÆ access controlled via admin_permissions rows.
...
COMMENT ON TABLE admin_permissions IS
  'Per-section permissions for editor/viewer admins.
   Superadmin rows do NOT need entries here ΓÇö full access by RLS policy.';
...
-- ============================================================
-- TABLE: proposals
-- CHANGE 1: Polymorphic source_type/source_id ΓåÆ explicit FKs
...
BEGIN
  RAISE EXCEPTION 'audit_logs is append-only ΓÇö UPDATE and DELETE are not permitted.';
END;
$$;
...
-- CHANGE 1: REMOVED ΓÇö fn_cascade_entity_media trigger (no longer needed, native FK CASCADE handles it)
-- CHANGE 1: REMOVED ΓÇö fn_nullify_proposal_source trigger (no longer needed, native FK SET NULL handles it)
