...
export default function AuditLogsManager() {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get<any>(`/api/v1/admin/audit_logs?${queryParams.toString()}`);
    ...
    <div className="space-y-8 font-space text-white relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="px-5 py-2.5 rounded-full text-xs font-black uppercase tracking-wider text-white bg-slate-800 hover:bg-slate-700 border border-white/5 hover:border-white/10 transition-all flex items-center gap-1.5 self-start disabled:opacity-50"
        >
      ...
      <div className="flex flex-col sm:flex-row gap-4 p-4 rounded-3xl bg-slate-900/40 border border-white/5 backdrop-blur-md">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <select
            ...
            className="px-4 py-2 bg-slate-950 border border-white/5 focus:border-blue-500/40 rounded-2xl text-xs text-slate-350 focus:outline-none [&>option]:bg-[#0a1122]"
          >
      ...
      {/* Logs Table */}
      <div className="p-6 rounded-3xl bg-slate-900/40 border border-white/5 backdrop-blur-md shadow-xl overflow-hidden min-h-[300px] flex flex-col">
        {loading ? (
          <div className="flex-1 flex flex-col items-center justify-center text-slate-400 gap-3 py-16">
            <Loader2 className="w-8 h-8 text-blue-500 animate-spin" />
            <span className="text-xs font-bold uppercase tracking-wider">Loading System Audits...</span>
          </div>
        ) : (
                  ...
                  logs.map((log) => (
                    <tr key={log.id} className="hover:bg-white/[0.01] transition-all">
                      <td className="py-4 px-4 whitespace-nowrap text-slate-500 flex items-center gap-1.5">
                        ...
                        <span>{log.created_at.replace("T", " ").slice(0, 19)}</span>
                      </td>
                      <td className="py-4 px-4 min-w-[150px]">
                        <div className="flex flex-col">
                          <span className="text-white leading-tight">{log.user_name}</span>
                          <span className="text-[10px] text-slate-500 leading-none mt-1 font-mono font-medium">{log.user_email}</span>
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-4 font-mono text-[10px] text-slate-400">{log.entity || "N/A"}</td>
