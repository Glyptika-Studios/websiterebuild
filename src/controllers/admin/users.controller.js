...
export const inviteAdminUser = asyncHandler(async (req, res) => {
  ...
  // Validate sections if editor or viewer
  if (role !== "superadmin") {
    if (!sections || sections.length === 0) {
      throw new ApiError(
        400,
        "sections is required for editor and viewer roles"
      );
    }
  ...
  if (role !== "superadmin" && sections && sections.length > 0) {
    const permissionRows = sections.map((s) => ({
      user_id:   userId,
      section:   s.section,
      can_read:  true,
